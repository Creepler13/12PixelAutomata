...
function drawUI() {
    ...
    for (let index = 0; index < ids.length; index++) {
        ...
        ctx.drawImage(World.TileCreator.getTexture("materials", ids[index], 1), 50, 600 + (index + 1) * 12, 8, 8);
    ...
    for (let index = 1; index < World.TileCreator.getEntrySize("buildings", getSelectet()) + 1; index++) {
        ctx.drawImage(World.TileCreator.getTexture("buildings", getSelectet(), index), 300, 600 + index * 13, 12, 12);
        if (World.TileCreator.getData("buildings", getSelectet(), index, "price") > World.Materials.get("water")) {
            ctx.fillStyle = "#ff0000"
        }
        ctx.fillText(World.TileCreator.getData("buildings", getSelectet(), index, "price"), 300 + 15, 600 + index * 12 + 12);
    }
}
...
document.onkeyup = function KeyEventHandler(e) {
    ...
    switch (code) {
        ...
        case 32: //space
            switch (tool) {
                case 0:
                    if (World.TileCreator.getData("buildings", getSelectet(), 1, "price") > World.Materials.get("water") && getSelectet() == World.TileCreator.getData("buildings", World.get(x, y).type)) break;
                    World.Materials.set("water", World.Materials.get("water") - World.TileCreator.getData("buildings", getSelectet(), 1, "price") + World.TileCreator.getData("buildings", World.get(x, y).type, World.get(x, y).level, "price") / 2);
                    World.set(x, y, "buildings", getSelectet(), facing, 1);

                    break;
                case 1:
                    console.log(World.TileCreator.getEntrySize("buildings", World.get(x, y).type))
                    if (World.TileCreator.getEntrySize("buildings", World.get(x, y).type) <= World.get(x, y).level) break;
                    if (World.TileCreator.getData("buildings", World.get(x, y).type, World.get(x, y).level + 1, "price") > World.Materials.get("water")) break;
                    World.Materials.set("water", World.Materials.get("water") - World.TileCreator.getData("buildings", getSelectet(), World.get(x, y).level + 1, "price") + World.TileCreator.getData("buildings", getSelectet(), World.get(x, y).level, "price") / 2)
                    World.levelUp(x, y);
                    return;
                case 2:
                    World.Materials.set("water", World.Materials.get("water") + World.TileCreator.getData("buildings", getSelectet(), World.get(x, y).level, "price") / 2);
